...
// From BIQU's motor_ledc_timer_init: speed_mode 1,
// duty_resolution 10 bits, freq_hz 30000 (literal 0x7530).
...
// The startup ramp.
//
// The motor does NOT go straight to full duty. It starts at 102 of 1023, about
// ten percent, and ramps to full over 20 ms. Before any of that it drops the
// opposite channel, waits a tick, and refuses to drive at all unless that
// channel reads back zero -- see group_drive, where the sequence lives.
//
// The dead time is the smallest of the four barriers and not the one that
// makes this safe: at CONFIG_FREERTOS_HZ=100 the tick before it is 10 ms,
// twenty times longer, and the duty readback between them is what actually
// guarantees the bridge let go. See private/SPEC/motor-drive.md section 3b.
#define PWM_START_DUTY   102
#define PWM_FADE_MS      20
#define PWM_STOP_FADE_MS 10
#define PWM_DEADTIME_US  500
...
// before doing anything: moving, direction
// (1 = fwd_chan, 0 = rev_chan) and target hall state.
static bool s_grp_moving[PV_MOTOR_GROUPS];
static bool s_grp_fwd[PV_MOTOR_GROUPS];
static int  s_grp_target[PV_MOTOR_GROUPS];
// Stock's four per-group fault bytes. ORs them into
// the byte once per motor-task pass (called, right
// after the per-group loop), and the rgb task reads that byte through
// to raise the red strobe.
//
// The predicate is RECOVERED, 2026-08-28. It is not a stall guard and it is
// not the >= 4 threshold. is the in-progress path for group i; it
// runs only while the group's moving byte is set, and only once
// its per-group timestamp has aged past the same 199 literal the
// drive loop uses. At that re-check:
//
//   arrived, target == hall: stop, clear the retry count, clear moving,
//                             clear fault
//   not yet:                  raise fault, and once the retry count reaches
//                             MOTOR_FAULT_GIVEUP stop the group and clear
//                             moving WITHOUT clearing fault
//
...
// Is a drive actually needed?
//
// Three pieces of state have to agree before the answer is no: the group is
// already moving, already moving the right way, and already aimed at the
// target being asked for. Any one of them wrong means drive; all three right
// means return without touching the channels, because re-issuing the sequence
// on a group that is already doing the job would drop and re-raise its drive
...
// BIQU's stop helper. Fades the channel that is actually running
...
// BIQU's drive helpers (fwd) and (rev). One function
...
// Stock does NOT classify raw ADC counts. It takes the raw reading, passes
// it through adc_cali_raw_to_voltage, and classifies the resulting
// MILLIVOLTS. The bands 640..960,
...
// BIQU's motor loop, from the task.
//
...
// 200 ms. Stock compares against 199 and acts once the elapsed time passes
// it, so the effective period is 200 ms and that is what is written here.
#define MOTOR_TICK_MS   200

// Button timings.
#define PV_BTN_POLL_MS           10     // one tick at CONFIG_FREERTOS_HZ=100
#define PV_BTN_LONG_MS           2999   // long press at three seconds
#define PV_BTN_CLICK_SETTLE_MS   300    // ignore a second edge inside 300 ms
        ...
        for (int i = 0; i < s_groups; ++i) {
            ...
            if (st != target) {
                ...
                if (s_grp_seen[i]) {
                    ...
                    // still moving, which is every travel longer
...
//
                    ...
                    if (++s_grp_retry[i] >= MOTOR_FAULT_GIVEUP) {
                        //: stop and clear moving, leaving fault set.
                        s_grp_fault[i] = true;
                        group_stop(i);
                        moving[i] = false;
                        s_grp_seen[i] = false;
                ...
                // Arrival clears the retry count, the moving byte and the
                // fault byte together.
                s_grp_retry[i] = 0;
                s_grp_fault[i] = false;
                s_grp_seen[i] = false;
...
static void button_task(void *arg)
{
    ...
    // BIQU's button_task. Their timings, read from the image:
//
    //   poll interval        10 ms   (vTaskDelay(1))
    //   long press fires at  > 2999 ms  (literal = 0xbb7)
    //   a single click is dispatched only after the line has been released
    //   for more than 300 ms (0x12c), so a double tap does not
...
void pv_motor_start(void)
{
    ...
    if (err == ESP_OK) {
        // The fade service has to be installed once, after the timer and
        // before any ledc_fade_start. Without it ledc_fade_start returns
