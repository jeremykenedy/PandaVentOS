#!/usr/bin/env bash
#
# PandaVent OS restorer. Puts the device back the way it was.
#
...
#
# --factory writes BIQU's published application image. It does NOT restore your
# settings, because the stock firmware does not understand the configuration
# this one writes and will replace it with its own defaults on first boot.
#
...
FACTORY_BIN="$ROOT/factory/firmware/panda_vent_v1.0.0.bin"
MODE=""
IMAGE=""
PORT=""
HOST=""
...
if [[ "$MODE" == "factory" ]]; then
    [[ -f "$FACTORY_BIN" ]] || die "BIQU's image is not in this checkout at $FACTORY_BIN"
    SIZE=$(wc -c < "$FACTORY_BIN" | tr -d ' ')

    say "Restoring BIQU's firmware v1.0.0"
    info "$FACTORY_BIN"
    info "$SIZE bytes"
